...
export const slugify = (s: any) => {
  ...
  const latinized = lowered.latinise();
  const withDashes = latinized.replace(/[^a-z0-9]+/g, '-');
  const withoutLeadingTrailingDashes = withDashes.replace(/^-+|-+$/g, '');
  const withoutRepeatingDashes = withoutLeadingTrailingDashes.replace(/-+/g, '-');

  return encodeURIComponent(withoutRepeatingDashes);
}
